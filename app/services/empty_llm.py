...
class LLMService:
    ...
    async def generate_answer_rag_v2(
        self,
        query: str,
        ...
        """Generate a coherent answer using context chunks, query, and conversation history"""
        try:
            # Combine context chunks
            context = "\n\n".join([chunk["text"] for chunk in context_chunks])
            
            # Create enhanced French prompt template
            prompt_template = ChatPromptTemplate.from_messages([
                SystemMessage(content="""Tu es un assistant pédagogue expert qui génère des réponses précises et utiles basées sur le contexte fourni.
            ...
            # Save messages if session_id is provided
            if session_id:
                await self.mongo_service.save_message(session_id, "user", query)
                await self.mongo_service.save_message(session_id, "assistant", response)
                
                # Update conversation store
                if session_id not in self.conversation_store:
                    self.conversation_store[session_id] = []
                self.conversation_store[session_id].extend([
